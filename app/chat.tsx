import {
  ...
  Modal, Animated, Alert, ActivityIndicator, Share,
} from 'react-native';
...
import { 
  Plus, Mic, ArrowUp, Image, Moon, Target, Paperclip,
  Sun, MoonStar, Sunrise, Sunset, Smile, FileText, Brain, PenTool,
  MoreHorizontal, RefreshCcw, Copy, ThumbsUp, ThumbsDown, Volume2,
  X, ChevronRight,
...
export default function Chat() {
  ...
  const lang: 'ar' | 'en' = 'ar'; // يمكن ربطها بـ useTwinStore لاحقاً
  const isRTL = lang === 'ar';
  ...
  const send = async (messageText?: string) => {
    ...
    try {
      ...
      setEmotion(res?.emotion?.primary ?? 'neutral');
          ...
          if (res?.importance > 0.7) showToast("لحظة مميزة مع توأمك", "success");
  ...
  const handleMessageLongPress = (msg: any) => {
    Alert.alert(
      ...
      '',
      [
        { text: lang === 'ar' ? 'إعادة صياغة' : 'Rephrase', onPress: () => {} },
        { text: lang === 'ar' ? 'قراءة' : 'Read Aloud', onPress: () => {} },
        { text: lang === 'ar' ? 'إلغاء' : 'Cancel', style: 'cancel' },
      ]
    );
  };

  // ── Quick suggestion press ──────────────────────────────────
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          style={[
            styles.bubble,
            isUser ? styles.userBubble : styles.twinBubble,
          ]}
        >
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={styles.inputBar}>
          ...
          {/* زر الإرسال (يظهر فقط عند وجود نص) */}
          {input.trim().length > 0 ? (
...
const styles = StyleSheet.create({
  ...
  suggestionBtn: {
    backgroundColor: COLORS.card, borderRadius: 16, padding: 16, width: '45%',
